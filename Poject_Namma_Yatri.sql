...
select count(customer_not_cancelled) from trips_details4
where customer_not_cancelled=0;
-- average distance per trip
select * from trips;
select sum(distance)/count(distance) from trips;



-- average fare per trip

select sum(fare)/count(fare) from trips;
-- or
...
JOIN trips t ON p.id = t.faremethod
GROUP BY p.method
...
-- which 2 locations had most number of trips
select * from
(select * , dense_rank() over (order by cnt desc) rnk from
(select loc_from,loc_to, count(distinct tripid) cnt from trips
...
select sum( searches_for_quotes)*100.0/sum(searches_got_estimate) rate from trips_details4;
select * from trips_details4;

-- quote acceptance rate
select sum(searches_got_quotes)*100.0/sum(searches_for_quotes) rate from trips_details4;

-- quote to booking rate
select sum(customer_not_cancelled)*100.0/sum(searches_got_quotes) rate from trips_details4;



-- booking cancellation rate
select (count(*) -sum(customer_not_cancelled))*100.0/sum(searches_for_quotes) rate from trips_details4;

-- conversion rate
select sum(driver_not_cancelled)*100.0/sum(searches) rate from trips_details4;

-- which area got highest trips in each duration
select * from
(select *, dense_rank() over (partition by duration order by cnt desc) rnk from(
select loc_from,duration,count(distinct tripid) cnt from trips
group by loc_from , duration)a)b
where rnk=1;

-- which duration got highest trips in each location
select * from
(select *, dense_rank() over (partition by loc_from order by cnt desc) rnk from(
select loc_from,duration,count(distinct tripid) cnt from trips
group by loc_from , duration)a)b
where rnk=1;
...
select * from
(select *, dense_rank() over ( order by cancellation desc) rnk from(
select loc_from, count(*)-sum(driver_not_cancelled) cancellation from trips_details4
group by loc_from )a)b
where rnk=1;

select * from
(select *, dense_rank() over ( order by trip desc) rnk from(
select loc_from,count(distinct tripid) trip from trips
group by loc_from )a)b
where rnk=1;
